...
// File: @openzeppelin/contracts-upgradeable@4.8.3/token/ERC20/utils/SafeERC20Upgradeable.sol
    ...
    UUPSUpgradeable
{
    ...
    function swapNFT(uint256 tokenIdIn, uint256 tokenIdOut) external nonReentrant onlyInitialized whenNotPaused {
        if (IERC721(nftCollection).ownerOf(tokenIdOut) != address(this)) revert TokenUnavailable();

        uint256 totalFee = swapFee;
        if (totalFee > 0) {
            IERC20Upgradeable(feeToken).safeTransferFrom(msg.sender, address(this), totalFee);
            uint256 stonerAmount = (totalFee * stonerShare) / 100;
            if (stonerAmount > 0) {
                IERC20Upgradeable(feeToken).safeTransfer(stonerPool, stonerAmount);
            }
        }
    ...
    /// @dev Register on Sonic FeeM (optional)
