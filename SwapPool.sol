...
contract SwapPool is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function swapNFT(uint256 tokenIdIn, uint256 tokenIdOut) external nonReentrant onlyInitialized whenNotPaused {
        ...
        if (totalFee > 0) {
            IERC20(feeToken).safeTransferFrom(msg.sender, address(this), totalFee);
            uint256 stonerAmount = (totalFee * stonerShare) / 100;
            if (stonerAmount > 0) {
                IERC20(feeToken).safeTransfer(stonerPool, stonerAmount);
            }
        }
